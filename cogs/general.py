...
class General(commands.Cog):
    ...
    async def custom_help(self, ctx):
        ...
        embed.add_field(name="`!add_both <할 일> <D-day 숫자|week>`", value="마감일을 지정해 JFDI와 자신의 TLITODOS에 함께 추가합니다. D-day는 필수입니다.", inline=False)
        embed.add_field(name="`!sync_tli <ID>`", value="마감일이 설정된 기존 JFDI 할 일을 자신의 TLITODOS에 생성하거나 연결된 항목을 갱신합니다.", inline=False)
        ...
        embed.add_field(name="`!delete <ID>`", value="목록에서 할 일을 삭제합니다. TLITODOS에 연결된 항목은 양쪽에서 삭제합니다.", inline=False)
        ...
        embed.add_field(name="`!difficulty <ID> <1~5>`", value="목표의 작업 난이도를 1~5 숫자로 설정합니다. (취미는 설정 불가)", inline=False)
        embed.add_field(name="`!tt add <요일> <시간> <과목/내용>`", value="개인 시간표(루틴)에 정기적인 작업을 추가합니다.", inline=False)
        embed.add_field(name="`!tt list [@유저]`", value="자신(혹은 멘션한 유저)의 개인 시간표를 확인합니다.", inline=False)
        embed.add_field(name="`!tt remove <요일> <시간>`", value="개인 시간표에서 특정 항목을 삭제합니다.", inline=False)
